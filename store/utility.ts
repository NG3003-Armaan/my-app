...
export function setInvalidFields<TSTate extends { invalidFields: Record<string, unknown> }, TAction>(
  state: TSTate,
  action: PayloadAction<TAction>
) {
  state.invalidFields = _.merge(_.cloneDeep(state.invalidFields), action.payload)
}

export function createSetState<TKey extends keyof TState, TState extends object>(
  key: TKey,
  validation?: (value: TState[TKey]) => boolean
): (state: TState, action: PayloadAction<TState[TKey]>) => void
export function createSetState<TState extends object, TKey extends keyof TState, TNestedKey extends keyof TState[TKey]>(
  key: TKey,
...
export function createSetState<TKey extends keyof TState, TNestedKey extends keyof TState[TKey], TState extends object>(
  key: TKey,
  nestedKey?: TNestedKey,
